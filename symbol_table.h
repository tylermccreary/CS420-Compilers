...
public:	
  ...
  */
  ...
  void set_parm_pos_update (int &parm_pos, expr_type standard_type_type);
  
  // Convert an expr_type to a string - useful for debugging.
  string *to_string (expr_type t);

  /* Prints out all the data directives in the symbol table */
  void data_directives ();
 ...
 private:
  ...
  //print information everytime the symbol table is updated
  void print_stab_entry(string *id, string *env, expr_type t, int pos);
